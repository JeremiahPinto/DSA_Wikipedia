	...
	public int getShortestHamiltonianPath() {
		...
		for (int i = n - 1; i >= 0; i--) {
			...
			for (int j = 0; j < n; j++) {
				if ((cur & 1 << j) != 0
						&& (bj == -1
						|| dp[cur][bj] + (last == -1 ? 0 : page.dist(bj,last)) > dp[cur][j] + (last == -1 ? 0 : page.dist(j,last)))) {
					bj = j;
				}
			}
			order[i] = bj;
		...
		for(int i = 0; i < order.length; i++) System.out.print(page.getVertex(order[i])+ " ");
...
//		System.out.println(Arrays.toString(order));
	...
	public static void printGraph2(Graph G)
	{
		for(LinkedList<Integer> s: G.page)
		{
		for(Integer p: s)
...
class Graph
{
	...
	{
		if(page.get(a).contains(b)) return 1;
